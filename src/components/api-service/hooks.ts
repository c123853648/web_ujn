...
const getDialogUrlWithToken = (dialogId: string) => {
  const { protocol, host } = window.location;
  return `${protocol}//${host}/chat/share/knowledge?dialogId=${dialogId}`;
};
...
export const usePreviewKnowledge = (dialogId: string, idKey: string) => {

  const open = useCallback(
      (t: string) => {
        window.open(
            getDialogUrlWithToken(dialogId),
            '_blank',
        );
      },
      [idKey],
  ...
  const handlePreviewKnowledge = useCallback(async () => {
    const token = dialogId;
    if (token) {
      open(token);
    }
  }, [open]);
